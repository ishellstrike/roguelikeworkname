    ...
    public class CraftData
    {
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < Output.Count; i++) {
                sb.AppendLine(string.Format("{1}{0}", idb.Data[Output[i]].Name, OutputCount[i] != "1" ? OutputCount[i] != "0" ? OutputCount[i] + " " : "(и) " : string.Empty));
            }
        ...
        private void GetValue(StringBuilder sb, List<string> input, List<string> inputcount, int count)
        {
            if (input != null) {
                ...
                sb.Append(string.Format("{1}{0}", idb.Data[input[0]].Name, inputcount[0] != "1" ? inputcount[0] != "0" ? inputcount[0] + " " : "(и) " : string.Empty));
                for (int i = 1; i < input.Count; i++) {
                    sb.Append(string.Format(" или {1}{0}", idb.Data[input[i]].Name, inputcount[i] != "1" ? inputcount[i] != "0" ? inputcount[i] + " " : "(и) " : string.Empty));
                }
